...
do
  ...
  echo $(zcat $infile |wc -l)/4|bc >> $OUTDIR/trimmed/trimmed_read_stats_ecoli.txt
done
...
then
  ...
  mkdir $OUTDIR/ecoli_genome
  STAR --runThreadN 20 --genomeSAindexNbases 8 --runMode genomeGenerate \
  --genomeDir $OUTDIR/ecoli_genome --genomeFastaFiles $OUTDIR/ecoli_refseq.fa
...
do
  if [[ $prefix ]]; then
    if [ $analysis_mode == 'NONE' ] ; then
      STAR --runThreadN 24 --genomeDir $OUTDIR/ecoli_genome --outFileNamePrefix $OUTDIR/bams/"$outname" \
...
for infile in $OUTDIR/bams/"$outname"*_ecoli_q1.bam
do
