#!/bin/bash
#SBATCH --job-name=EED_ctrl
#SBATCH --partition=inter_p
#SBATCH --ntasks=1
#SBATCH --mem=1gb
#SBATCH --time=01:00:00
#SBATCH --output=/scratch/ry00555/RNASeqPaper2026/Proteome/SingleBaitPools/logs/EED_ctrl.%j.out
#SBATCH --error=/scratch/ry00555/RNASeqPaper2026/Proteome/SingleBaitPools/logs/EED_ctrl.%j.err
...
# EED_AF3_Pools.sh
#
# Orchestrates MSA -> INF for the EED single-bait screen (1268 pools).
#
...
INF_THROTTLE=4
INF_BATCH=8
MSA_SCRIPT="EED_AF3_MSA_Pool.sh"
INF_SCRIPT="EED_AF3_INF_Pool.sh"
INPUT_DIR="/scratch/ry00555/RNASeqPaper2026/Proteome/SingleBaitPools/EED/EED_PooledPPI_InputJSONs"
OUTPUT_DIR="/scratch/ry00555/RNASeqPaper2026/Proteome/SingleBaitPools/EED/EED_PooledPPI_output"
...
if [ "$MODE" == "start" ]; then
    ...
    echo "Submitting full MSA array for EED: 1-${TOTAL}%${MSA_THROTTLE}"
...
if [ "$MODE" == "inf_scan" ]; then
    echo "Scanning EED for pools ready for INF..."
    line_num=0
    ready=()
    ...
    echo "Found ${#ready[@]} EED pools ready for INF"

    if [ ${#ready[@]} -eq 0 ]; then
        msa_running=$(squeue --me --partition=batch --name=EED_MSA --noheader | wc -l)
...
if [ "$MODE" == "recheck" ]; then
    echo "Running missing-pools check for EED..."
    missing_msa=()
    missing_inf_only=()

    for i in $(seq 1 $TOTAL); do
        pool_glob="${OUTPUT_DIR}/eed_pool$(printf '%03d' $i)"
        # pool numbers >=1000 aren't zero-padded past 3 digits by the generator
        [ ! -d "$pool_glob" ] && pool_glob="${OUTPUT_DIR}/eed_pool${i}"
        dir="$pool_glob"
        has_msa=0
        has_inf=0
