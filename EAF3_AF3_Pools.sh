#!/bin/bash
#SBATCH --job-name=EAF3_ctrl
#SBATCH --partition=inter_p
#SBATCH --ntasks=1
#SBATCH --mem=1gb
#SBATCH --time=01:00:00
#SBATCH --output=/scratch/ry00555/RNASeqPaper2026/Proteome/SingleBaitPools/logs/EAF3_ctrl.%j.out
#SBATCH --error=/scratch/ry00555/RNASeqPaper2026/Proteome/SingleBaitPools/logs/EAF3_ctrl.%j.err
...
# EAF3_AF3_Pools.sh
#
# Orchestrates MSA -> INF for the EAF3 single-bait screen (1182 pools).
#
...
OUTPUT_DIR="/scratch/ry00555/RNASeqPaper2026/Proteome/SingleBaitPools/EAF3/EAF3_PooledPPI_output"
...
if [ "$MODE" == "start" ]; then
    ...
    echo "Submitting full MSA array for EAF3: 1-${TOTAL}%${MSA_THROTTLE}"
...
if [ "$MODE" == "inf_scan" ]; then
    echo "Scanning EAF3 for pools ready for INF..."
    line_num=0
    ready=()
    ...
    echo "Found ${#ready[@]} EAF3 pools ready for INF"

    if [ ${#ready[@]} -eq 0 ]; then
        msa_running=$(squeue --me --partition=batch --name=EAF3_MSA --noheader | wc -l)
...
if [ "$MODE" == "recheck" ]; then
    echo "Running missing-pools check for EAF3..."
    missing_msa=()
    missing_inf_only=()

    for i in $(seq 1 $TOTAL); do
        pool_glob="${OUTPUT_DIR}/eaf3_pool$(printf '%03d' $i)"
        # pool numbers >=1000 aren't zero-padded past 3 digits by the generator
        [ ! -d "$pool_glob" ] && pool_glob="${OUTPUT_DIR}/eaf3_pool${i}"
        dir="$pool_glob"
        has_msa=0
        has_inf=0
