#!/bin/bash
#SBATCH --job-name=SET7_ctrl
#SBATCH --partition=inter_p
#SBATCH --ntasks=1
#SBATCH --mem=1gb
#SBATCH --time=01:00:00
#SBATCH --output=/scratch/ry00555/RNASeqPaper2026/Proteome/SingleBaitPools/logs/SET7_ctrl.%j.out
#SBATCH --error=/scratch/ry00555/RNASeqPaper2026/Proteome/SingleBaitPools/logs/SET7_ctrl.%j.err
...
# SET7_AF3_Pools.sh
#
# Orchestrates MSA -> INF for the SET7 single-bait screen (1624 pools).
#
...
INF_SCRIPT="SET7_AF3_INF_Pool.sh"
INPUT_DIR="/scratch/ry00555/RNASeqPaper2026/Proteome/SingleBaitPools/SET7/SET7_PooledPPI_InputJSONs"
OUTPUT_DIR="/scratch/ry00555/RNASeqPaper2026/Proteome/SingleBaitPools/SET7/SET7_PooledPPI_output"
...
if [ "$MODE" == "start" ]; then
    ...
    echo "Submitting full MSA array for SET7: 1-${TOTAL}%${MSA_THROTTLE}"
...
if [ "$MODE" == "inf_scan" ]; then
    echo "Scanning SET7 for pools ready for INF..."
    line_num=0
    ready=()
    ...
    echo "Found ${#ready[@]} SET7 pools ready for INF"

    if [ ${#ready[@]} -eq 0 ]; then
        msa_running=$(squeue --me --partition=batch --name=SET7_MSA --noheader | wc -l)
...
if [ "$MODE" == "recheck" ]; then
    echo "Running missing-pools check for SET7..."
    missing_msa=()
    missing_inf_only=()

    for i in $(seq 1 $TOTAL); do
        pool_glob="${OUTPUT_DIR}/set7_pool$(printf '%03d' $i)"
        # pool numbers >=1000 aren't zero-padded past 3 digits by the generator
        [ ! -d "$pool_glob" ] && pool_glob="${OUTPUT_DIR}/set7_pool${i}"
        dir="$pool_glob"
        has_msa=0
        has_inf=0
